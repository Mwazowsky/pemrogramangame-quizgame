...
public class GameManager : MonoBehaviour
{
    ...
    private Question[] _relatedQuestions = null;
    public Question[] Questions { get { return _relatedQuestions; } }
    ...
    void Display()
    {
        ...
        if (events.UpdateNarration != null)
        {
            events.UpdateNarration(question);
        }
        else
        {
            Debug.LogWarning("Ups! Something went wrong while trying to display new Knowledge UI Data. GameEvents.UpdateExplanationUI is null. Issue occured in GameManager.Accept() method.");
        }
    }
    /// <summary>
    ...
    public void Accept()
    {
        ...
        if (type != UIManager.ResolutionScreenType.Finish)
        {
            ...
            if (isKnowledgeScene == true)
            {

                if (events.UpdateExplanationUI != null)
                {
                    events.UpdateExplanationUI(Questions[currentQuestion]);
                }
                else
                {
                    Debug.LogWarning("Ups! Something went wrong while trying to display new Knowledge UI Data. GameEvents.UpdateExplanationUI is null. Issue occured in GameManager.Accept() method.");
    ...
    public void PlayNarrationMan()
    {
        if (events.PlayNarration != null)
        {
            events.PlayNarration(Questions[currentQuestion]);
        }
        else
        {
            Debug.LogWarning("Ups! Something went wrong while trying to display new Knowledge UI Data. GameEvents.UpdateExplanationUI is null. Issue occured in GameManager.Accept() method.");
    ...
    void LoadQuestions()
    {
        ...
        List<Question> _relatedQuestionsList = new List<Question>();
        for (int i = 0; i < objs.Length; i++)
        {
            _questions[i] = (Question)objs[i];
            if (_questions[i].GetQuestionType.ToString() == SceneManager.GetActiveScene().name)
            {
                _relatedQuestionsList.Add(_questions[i]);
            }
        }
        _relatedQuestions = _relatedQuestionsList.ToArray();
    }
